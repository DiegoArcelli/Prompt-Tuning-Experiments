...
class MT5Attention(nn.Module):
    ...
    def forward(
        ...
        # print(scores.shape, position_bias_masked.shape,)
...
class MT5LayerSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if encoder_prompt is not None:
            hidden_states = torch.concat((hidden_states, encoder_prompt.unsqueeze(0).repeat(hidden_states.size(0), 1, 1)), dim=1)
        elif decoder_prompt is not None:
            hidden_states = torch.concat((hidden_states, decoder_prompt.unsqueeze(0).repeat(hidden_states.size(0), 1, 1)), dim=1)
...
class MT5Block(nn.Module):
    ...
    def forward(
        ...
        if hidden_states.dtype == torch.float16 and torch.isinf(hidden_states).any():
            ...
            hidden_states = torch.cla            # print(decoder_prompt.shape)
            # print(attention_mask[0, 0])mp(hidden_states, min=-clamp_value, max=clamp_value)
...
class MT5Stack(MT5PreTrainedModel):
    ...
    def forward(
        ...
        for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
            ...
            if encoder_prompt is not None:
                prompt_tokens = encoder_prompt.shape[0]
                batch_size, tokens, _ = hidden_states.shape           
                new_attention_mask = torch.full((batch_size, 1, 1, prompt_tokens+tokens), extended_attention_mask[0,0,0,0])
                new_attention_mask[:, :, :, -tokens:] = extended_attention_mask
                extended_attention_mask = new_attention_mask
            elif decoder_prompt is not None:
                prompt_tokens = decoder_prompt.shape[0]
                batch_size, tokens, _ = hidden_states.shape
                length = tokens
                if not (past_key_values is None or  past_key_values[0] is None):
                    last_length = past_key_values[0][0].shape[2]
                    tokens_gen = last_length//(prompt_tokens+1)
                    length += tokens_gen
                if extended_attention_mask.shape[-1] == extended_attention_mask.shape[-2] and tokens != 1:
                    new_attention_mask = torch.full((batch_size, 1, prompt_tokens+length, prompt_tokens+length), extended_attention_mask[0,0,0,0])
                    if new_attention_mask.shape[-1] > extended_attention_mask.shape[-1]:
                        new_attention_mask[:, :, -length:, -length:] = extended_attention_mask
                else:
                    new_attention_mask = torch.full((batch_size, 1, 1, prompt_tokens+length), extended_attention_mask[0,0,0,0])
                    if new_attention_mask.shape[-1] > extended_attention_mask.shape[-1]:
                        new_attention_mask[:, :, :, -length:] = extended_attention_mask
                extended_attention_mask = new_attention_mask
...
class MT5ForConditionalGeneration(MT5PreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if labels is not None:
                batch_size, labels_num_tokens = labels.shape
                lm_logits_num_tokens = lm_logits.shape[1]
                if labels_num_tokens != lm_logits_num_tokens:
                    pad_tokens_num = lm_logits_num_tokens - labels_num_tokens
                    soft_prompts_indices = torch.full((batch_size, pad_tokens_num), -100).to(self.device)
                    labels = torch.concat([soft_prompts_indices, labels], dim=1)
